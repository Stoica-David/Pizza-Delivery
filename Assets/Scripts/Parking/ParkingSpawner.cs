...
public class ParkingZoneSpawner : MonoBehaviour
{
    [SerializeField]
    GameObject[] parkingZonePrefabs; // Prefab-urile pentru zonele de parcare

    GameObject[] parkingZonePool = new GameObject[40]; // Pool-ul de zone de parcare

    Transform playerCarTransform;

    // Timing
    float timeLastZoneSpawned = 0;
    WaitForSeconds wait = new WaitForSeconds(15.0f); // Interval de așteptare între generări
    ...
    void Start()
    {
        ...
        for (int i = 0; i < parkingZonePool.Length; i++)
        {
            parkingZonePool[i] = Instantiate(parkingZonePrefabs[prefabIndex]);
            parkingZonePool[i].SetActive(false);

            prefabIndex++;

            // Reîncepem de la începutul listei de prefabricate dacă am ajuns la capăt
            if (prefabIndex > parkingZonePrefabs.Length - 1)
                prefabIndex = 0;
        }
    ...
    void SpawnNewParkingZones()
    {
        ...
        foreach (GameObject parkingZone in parkingZonePool)
        {
